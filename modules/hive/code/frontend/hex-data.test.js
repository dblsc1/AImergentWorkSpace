...
// ══════════════════════════════════════════════════════════════
// 1. 「最近完成」去重 —— 本文件最重要的一组
//    实测流水里同一个任务被勾了又取消；漏了去重就是给人类显示假成绩。
// ══════════════════════════════════════════════════════════════
...
test("晶格：第 d 圈恰好 6d 格，格位角宽正好 60/d 度（15° 外延的几何来源）", function () {
  ...
  // 第 4 圈角宽正好 15° —— 人类那条「小于 15° 就外延」的阈值不是写死的常数，
  // 是这张表的第 4 行。
...
test("咬合（本轮的核心不变量）：相邻格中心距**恒等于** √3·S，不多不少", function () {
  ...
  // 人类判词「格子不咬合」。轴向晶格里相邻中心距恒为 √3·S = 六边形的对边宽，
...
test("不预留空位：有项目的分区**一个占位格都没有**（建项目走蜂巢外的空白）", function () {
  // 2026-09-08 人类改判：「每个分区都多了一个高饱和度色块……我要的是整个分区
...
// ── 冷区权重补贴（2026-09-14，人类：「给冷区（第二圈都排不上的）加一个权重补贴」）──
//
...
test("不许有孤岛：18° 的分区必须从内圈就开始填，不能被当成窄区推到外圈", function () {
  // 真机实测出来的 bug（截图 hexv2-01）：人类给的阈值是「角度小于 15 度才外延」，
...
test("虚拟格延伸：沿本该在那儿的六边形继续折出去，边长仍恒为 S", function () {
  // 人类看过径向直线那一版：「射线不够好看啊，做成沿着虚拟的六边型的格式」。
...
test("圆心读数：少于 1 小时 分:秒，多于 1 小时 时:分（人类 2026-09-12）", function () {
...
test("深浅：外浅内深，等比例，项目再多也落在固定区间里（人类 2026-09-12）", function () {
...
// ── 拖分区名 = 换绕圈角度（人类 2026-09-08：「不是机械的移动六边形」）──
