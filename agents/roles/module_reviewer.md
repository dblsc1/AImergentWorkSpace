...
## 不重复造轮子（2026-07-30 用户裁定）

审到「流程/机制该怎么设计」类问题时，**先查前人成果再下判、再提修法**——
对照报告在 `agents/reference/前人成果对照-多agent框架.md`（含来源链接）。按用途速查：
Devin（检查单/记忆/多实例管理）、OpenHands（事件流留痕）、MetaGPT（结构化交接）、
AgentCoder（测试职责分离）、agentic-os（证据式完成）、MS Agent Governance Toolkit（三层闸）。
判自造方案时追问两句：**①这问题在参考项目里怎么解的？②本场景哪里不同到非自造不可？**
答不出②的自造件，判为「重复造轮子」打回；真空白的自造件，要求附替代方案对比。
