...
4. **测试覆盖（必须记录结论，但不硬拦）**：本区间新增了功能代码却没有配套测试时，
   `review_start.sh` 会提示你。**有些改动确实不需要新增测试**——改配置、改文案、重构无行为变化。
   所以这条不是硬性拦截，但**你必须在报告里明确写出结论：需要补 / 不需要补 + 理由**。
   写"不需要补"是可以的，写不出理由就是漏审。
   **你自己的测试义务（裁决 J2）**：programmer 的单测只覆盖它自己的视角；
   你负责**整合级测试**——跨整个代码文件/子模块的行为验证与消费方契约测试，
   落 `review/reviewcode/tests/`（pytest 可发现），挂进 `run_all.sh`。
   push 前的全量测试（`scripts/gates/run-tests.sh`）会连你这层一起跑（J5）。
5. **留痕齐不齐**：worklog、`report.json` 是否已 commit 且内容与结论一致。
