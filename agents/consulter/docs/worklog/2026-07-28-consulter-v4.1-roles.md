...
## 「新功能必须带测试」降级为审核提示（人类修正）

原实现是硬闸门（`checks/programmer/83`），拒绝提交。**人类指出这会误伤**：
改配置、改文案、无行为变化的重构，确实不需要新增测试。

改法：从 `checks/` 撤掉，移到 `review_start.sh` —— 起审时自动检测并提示 reviewer，
`programmer_reviewer` 角色卡里写死**必须在报告给出结论：需要补 / 不需要补 + 理由**。
写"不需要补"是可以的，**写不出理由才是漏审**。

这是个好的一般化教训：**判断题不该做成硬闸门。** 硬闸门只适合"事实题"
（文件在不在、路径对不对、区间空不空）；一旦需要"看情况"，就该交给人/reviewer 记录结论，
硬拦的结果是逼人用逃生口，而逃生口用多了整套闸门就废了。
