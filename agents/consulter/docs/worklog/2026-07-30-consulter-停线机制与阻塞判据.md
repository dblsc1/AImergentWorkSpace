...
判例库立总则（先查前人再自造，两问判据：参考项目怎么解的/本场景哪里非自造不可），
六个按用途速查的参考项目清单入 记忆.md 与 module_reviewer 卡（审「机制设计」类
问题时先对照、答不出必要性即按重复造轮子打回）。

## 附3：汇报纪律入卡（用户裁定：不要抽象汇报）

给用户的汇报必须自足：代号/区间/裁决号首现带解释、可执行内容给完整粘贴版、
不假设用户记得先前状态。已入 consulter 角色卡「汇报」节并重生成子代理 prompt。

## 附4：「什么时候必须叫人」成文（用户口述 2.2 清账）

实证病根：清单不成文 → CFO 把本机开发容器决策上升到用户（流程没错，规范太保守）。
成文进 supervision.md：必须叫人仅五类（花钱/扩暴露面、不可逆破坏、裁决冲突、
双方参与、主观产品取舍）；绑 127.0.0.1 可一键回收的本机开发基建 = CFO 自决+记账，
reviewer 事后核记账；拿不准先做+显著记账，纠错靠判例修清单。细目待用户追认。
