...
## 沉默失败的「形状」：同一个坑的第二次（CFO 指出）

CFO 指出 D2 我只修对了名字，**`if [ -x … ]` 的沉默失败形状原样保留** ——
脚本再改名或丢失，照样静默跳过、照样打印「门禁 已安装」。它是对的：
**这正是铁律 23 说的「只修实例不修类」，而我本轮刚写下那条铁律。**

按类扫，全仓三处同形状，逐条按性质处置：

| 位置 | 性质 | 处置 |
|---|---|---|
| `new_module.sh` 调 install-gates | 关键路径 | **缺文件即 die**，且装完**回验 hook 与 gates 真实存在**（不信退出码） |
| `arbiter-push.sh` 调 run-gates | 安全闸门 | 缺文件即拒绝推送 |
| `install-gates.sh` 调面板 install | 可选件 | 允许跳过，但**必须打印「已跳过」** |
...
`arbiter-push.sh` 原先只校验有没有 approved 审核，**不跑 run-gates** —— 我自己就借这个洞
在门禁红的情况下推过一次。现已强制：不绿不推，逃生口 `AIMERGENT_PUSH_SKIP_GATES` 记账。
selftest 扩到 20 条。
