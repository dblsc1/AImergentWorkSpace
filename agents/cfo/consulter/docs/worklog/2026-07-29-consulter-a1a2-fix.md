...
在 CFO 工作树提交文档被 20-report-committed 误拒：它 find 全盘扫 report.json，
**越过嵌套模块仓边界**，把 code/nexus-core（独立仓）里的报告当根仓文件判"未跟踪"。
后果：**模块一存在，根仓就永远提交不了。** 检查 30/70 同形状。

类修复：三个检查统一加"文件所属仓 toplevel ≠ 本仓则跳过"守卫（嵌套仓归自己的门禁管）；
selftest 19 断言守类。监控实测有效：这次误拒是被我自己的 Monitor 抓到的。
