...
>
> **§3 的事件标准已被 `contracts/yq-event-v1.md` 取代。** 本文 §3 与 §6 的信封写法**已过时**：
> 真正的防重键是 `dedupeKey`（不是 `id`+`source`），信封有 `time` 与 `recordedAt` **两个**时间字段，
> `subject` 里 `tier2`/`tier3` 必填而 `project`/`task` 选填。**照本文 §3 实现会实现错。**
