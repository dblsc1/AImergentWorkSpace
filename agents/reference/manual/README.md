...
| `docs/` | 文档导航与权威架构说明 |
| `agents/roles/` | arbiter / backend / frontend / reviewagent 基线，以及 orchestration / report-schema 协议 |
| `agents/cfo/` | 项目 arbiter 与架构 consulter 的通用角色卡；不携带任何项目 findings |
| `scripts/` | `new_module.sh`、`install-ci.sh`、gates、hooks、workflow 与合并脚本 |
| `code/_template/` | 新模块骨架唯一事实源 |
| `<modules>/` | 项目自定的模块容器；示例占位：`modules/example_service/` |

## 模块内部结构

`AGENTS.md` + `codeagent/<角色>/` + `module_docs/` + `code/` + `review/`。完整骨架以 `code/_template/` 为准，不在手册里维护第二份列表。
