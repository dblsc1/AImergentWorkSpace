...
解除标志：`cockpit-ai-planner` @ `agents/cfo/docs/依赖索引.md`

（即依赖索引里 ai-planner 那行的「远端」列从「⛔ 无远端」改成真实远端名。
**这一步只有在 `git ls-remote origin` 真看到 SHA、且与本地 `feat/init` 一致之后才允许做** ——
改索引是 CFO 的动作，等于 CFO 签字确认「东西真的在远端了」。）

### ⚠️ 为什么标志不能是 `origin` @ `.git/config`（2026-08-11 自我更正）

初版我写的就是那个，**它是个假绿**：`git remote add` 是**纯本地操作、不联网**，
跑完 config 里立刻出现 `origin`，障签自动解除，**而数据仍然只在这台机器上**。

一个「配置了远端」的判据，证明不了「东西推上去了」。本障签存在的全部理由是后者。

这跟本项目反复踩的是同一个形状：**判据核的是「像不像做完了」，不是「有没有真做完」**
（同期实证：`15-stale-blocker.sh` 的解除标志解析恒假、`smoke.sh` 对会扩展的响应做精确相等断言）。
我在给别人挑这个毛病的同一个晚上，自己写了一个。记在这里。
