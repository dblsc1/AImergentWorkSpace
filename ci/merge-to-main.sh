...
verify_report() {
  ...
  jq -e --arg role "$expected_role" --arg branch "$branch" --arg path "$path" '
    ...
    (if $role == "reviewagent" then
       ...
       any(.git.changed_files[]; startswith("CFO_agent/consulter/docs/worklog/"))
     end) and
  ...
  while IFS= read -r changed; do
    ...
    if [ "$expected_role" = reviewagent ]; then
      ...
      case "$changed" in
        CFO_agent/consulter/docs/*|CFO_agent/arbiter/docs/*) ;;
        *) printf '  未经审核的后续路径: %s\n' "$changed" >&2; bad=1 ;;
      esac
    fi
...
if [ "$level" = module ]; then
  ...
  if verify_report CFO_agent/consulter/docs/findings/report.json consulter; then
    approved=1
  elif [ -n "$pr" ]; then
