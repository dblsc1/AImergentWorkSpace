...
if [ ! -f "$_rg" ]; then
  ...
  # ── D16 双向点火：C1 只管源码，散文不算（人类裁决 2026-08-02）────────
  # **同一份 1200 行内容，只改扩展名**。这是刻意的：任何「.md 不红」的旁因
  # （被当成二进制、被当成第三方产物、路径压根没进扫描集）都会让 .sh 那发也不红，
  # 于是两发一起挂。只验一半＝把门关掉一半而没人知道。
  mkdir -p "$_sb62/docs"
  _ext62() { { _i=0; while [ $_i -lt 1200 ]; do printf 'x\n'; _i=$((_i+1)); done; } > "$_sb62/$1"; git -C "$_sb62" add -A >/dev/null 2>&1; }
  _ext62 docs/prose.md; _o62e=$(_seg62)                             # 散文 1200 行 → 不该红   # ref-fixture
  rm -f "$_sb62/docs/prose.md"                                                                # ref-fixture
  _ext62 docs/prose.sh; _o62f=$(_seg62)                             # 同内容改 .sh → 必须红   # ref-fixture
  rm -rf "$_sb62"
  ...
  case "$_o62a" in *"个源码文件（跳过"*) ;; *) _v62="$_v62 [不打印被验对象]" ;; esac
  # D16 两个方向，缺一发都算门只关了一半：
  case "$_o62e" in *"❌"*"prose.md"*) _v62="$_v62 [.md 超1000仍被拦：D16 没落地]" ;; esac   # ref-fixture
  case "$_o62f" in *"❌"*"prose.sh"*) ;; *) _v62="$_v62 [同内容改.sh竟不红：白名单把源码也漏了]" ;; esac   # ref-fixture
  # 白名单会**缩小**扫描集，所以扫描计数必须随扩展名真的动。
  # 只断言「输出里有『不在 C1 治理范围』这几个字」是弱的——那行是常量，删掉被验逻辑照样印。
  # 这里比的是**同一沙箱前后两次的扫描数之差**：.md 那次不该进扫描集，.sh 那次该进，差必须正好 1。
  _n62e=$(printf '%s\n' "$_o62e" | sed -n 's/.*扫描 \([0-9]*\) 个源码文件.*/\1/p')
  _n62f=$(printf '%s\n' "$_o62f" | sed -n 's/.*扫描 \([0-9]*\) 个源码文件.*/\1/p')
  [ "$(( _n62f - _n62e ))" -eq 1 ] || _v62="$_v62 [扫描计数没随扩展名变(${_n62e:-空}→${_n62f:-空})：计数是化石]"
  if [ -z "$_v62" ]; then
    P "三档齐全：>1000 硬拦、D2 顶部标记只警告、501–1000 记不到那一笔就红、记到就放行，且打印被验对象；D16 范围双向点火：同一份 1200 行内容 .md 放行 / .sh 变红，扫描计数随之 $_n62e→$_n62f"
  else
    F "C1 判据失灵：$_v62"
  fi
fi
