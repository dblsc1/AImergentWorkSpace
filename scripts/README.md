...
**不用记 18 个脚本名。** `scripts/aim` 无参数时打印整条生命周期 + 每步该跑什么，
带参数时直接代跑：`aim start` / `aim run` / `aim done` / `aim review` / `aim push` …

脚本数量本身不是问题，**「我现在该跑哪个」没有答案**才是问题。
分目录只解决「东西放哪」，解决不了「下一步是什么」——所以入口先于分类。
...
| `mission_start.sh <角色> <任务单> <写区...> [--docs …]`（`--checklist` 看单子） | **起飞前检查单**（8 项）：校验任务单四小节 → 申领写区路签（前缀重叠即拒）→ **声明预期文档变更（完工时机器逐条核对）** → 出提示词。**发签与拒发都记账**：成功 `lease_grant`、被拒 `lease_denied`（带失败项）——只记成功那一半，问题规模就数不出来。**拒发时还印「能不能强收」的三条判据 + 可粘贴命令**（`lib/lease.sh` 的 `lease_takeover_advice`），但**永不自动强收** |
