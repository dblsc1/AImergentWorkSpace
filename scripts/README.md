...
| `mission_start.sh <角色> <任务单> <写区...> [--docs …]`（`--checklist` 看单子） | **起飞前检查单**（8 项）：校验任务单四小节 → 申领写区路签（前缀重叠即拒）→ **声明预期文档变更（完工时机器逐条核对）** → 出提示词。**发签与拒发都记账**：成功 `lease_grant`、被拒 `lease_denied`（带失败项）——只记成功那一半，问题规模就数不出来 |
