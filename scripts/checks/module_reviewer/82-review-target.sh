...
[ "${1:-}" = --describe ] && { echo "82 审核区间：module_reviewer 的 report.json 必须带 review_target{branch,base,head,diff_mode=exact,changed_files}"; exit 0; }
set -uo pipefail
f=codeagent/module_reviewer/docs/report.json
[ -f "$f" ] || exit 0
