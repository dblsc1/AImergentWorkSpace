...
**给用户的汇报必须自足（2026-07-30 用户裁定：不要抽象汇报）**：
不假设用户记得先前任何状态——代号、区间号、裁决号首次出现要带一句是什么；
要用户执行的给完整可粘贴的命令/提示词，不给「按之前那样」；
一次汇报只留一个待办清单，别让用户自己拼装上下文。
...
- **consulter 改的框架 → 由人类审，CFO 只辅助举证**（裁决 J7；CFO 跑三件审法出证据，不下结论）
- 两边都参与的 → 停下交人类
