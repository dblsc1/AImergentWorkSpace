...
> 报告由**三部分拼成**：①项目级通用（本文件）②角色特色（框架根 `roles/<角色>.md`）③模块特色（`<module>/module_docs/report.md`）。保持简单——CFO/人要一眼能读。
...
## ②③ 拼接
角色特色字段见框架根 `roles/<角色>.md` 的「报告特色字段」节；模块特色见模块 `module_docs/report.md`。三部分合成一个 json 对象。
