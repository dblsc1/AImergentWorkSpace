...
2. **续用 > 重开**:用当前 harness 的续用/追加任务能力联系已开子代理，让它保留自己的上下文。**打回-修复循环续用同一个实现 agent，不要每轮重开**。具体动作见 `roles/arbiter.md` 附录 harness 映射表（Claude Code = `SendMessage(agentId)`）——**"续用"是个动作，不是个态度**；照抄那张表，别自己发挥。
...
- **CI/CD**=push 后自动门禁（框架根 `ci/`：gates + tests），PR 到 main 时运行。
